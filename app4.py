...
def run_agentic_query(question, messages):
    ...
    M&A BENCHMARKING FORMULAS (CRITICAL MATHEMATICAL TRANSLATIONS):
    When the user asks for specific M&A calculations/metrics, map them EXACTLY to these SQL expressions (use CAST to avoid integer truncation):
    
    - "IT Spend % Revenue":
      SQL Expression: (CAST(primary_kpis.IT_Spend_m AS REAL) / primary_kpis.Revenue_m) * 100.0
      Example: SELECT Project_Name, (CAST(IT_Spend_m AS REAL) / Revenue_m) * 100.0 AS IT_Spend_Percent_Revenue FROM primary_kpis;
      
    - "Standalone Cost % Revenue":
      SQL Expression: (CAST(primary_kpis.IT_Standalone_Cost_m AS REAL) / primary_kpis.Revenue_m) * 100.0
      Example: SELECT Project_Name, (CAST(IT_Standalone_Cost_m AS REAL) / Revenue_m) * 100.0 AS Standalone_Cost_Percent_Revenue FROM primary_kpis;
      
    - "IT Spend per Employee":
      SQL Expression: (CAST(primary_kpis.IT_Spend_m AS REAL) * 1000000.0) / primary_kpis.Org_Size
      Example: SELECT Project_Name, (CAST(IT_Spend_m AS REAL) * 1000000.0) / Org_Size AS IT_Spend_Per_Employee FROM primary_kpis;
      
    - "IT Spend per IT FTE":
      SQL Expression: (CAST(primary_kpis.IT_Spend_m AS REAL) * 1000000.0) / primary_kpis.IT_Team_Size
      Example: SELECT Project_Name, (CAST(IT_Spend_m AS REAL) * 1000000.0) / IT_Team_Size AS IT_Spend_Per_IT_FTE FROM primary_kpis;
      
    - "Revenue per IT FTE":
      SQL Expression: (CAST(primary_kpis.Revenue_m AS REAL) * 1000000.0) / primary_kpis.IT_Team_Size
      Example: SELECT Project_Name, (CAST(Revenue_m AS REAL) * 1000000.0) / IT_Team_Size AS Revenue_Per_IT_FTE FROM primary_kpis;
      
    - "IT Team % Org Size":
      SQL Expression: (CAST(primary_kpis.IT_Team_Size AS REAL) / primary_kpis.Org_Size) * 100.0
      Example: SELECT Project_Name, (CAST(IT_Team_Size AS REAL) / Org_Size) * 100.0 AS IT_Team_Percent_Org_Size FROM primary_kpis;
      
    - "Capex % IT Spend" (Requires multi-table join):
      SQL Expression: (CAST(cost_breakdown.IT_capex_m AS REAL) / primary_kpis.IT_Spend_m) * 100.0
      Example: SELECT pk.Project_Name, (CAST(cb.IT_capex_m AS REAL) / pk.IT_Spend_m) * 100.0 AS Capex_Percent_IT_Spend FROM cost_breakdown cb JOIN primary_kpis pk ON cb.Project_Name = pk.Project_Name;
